...
MAX_LEN = 1024
NUM_CLASSES = 2
OUTPUT_DIR = "./finetuned_phi4_content_safety"

def create_model_and_tokenizer(tokenizer, device, local_rank, loss_fn=None):    
    model = Phi4ForSequenceClassification(MODEL_NAME, num_classes=2, loss_fn=loss_fn).to(device)
...
def main():
    ...
    optimizer = torch.optim.AdamW(
        ...
        lr=1e-3,      # higher LR is OK since only head trains
        weight_decay=0.01,
    )
